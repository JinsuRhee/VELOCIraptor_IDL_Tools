FUNCTION rv_ptmatch, settings, dir_data, data, n_snap, run=run
...
IF run EQ 2L THEN BEGIN
	...
	dmp_mass	= 1./(DOUBLE(settings.effective_resolution)^3) * $
	...
	REPEAT BEGIN
		...
		IF N_itr LT N_itrmax THEN BEGIN
			...
			;;----- Matching
			ftr_name	= settings.dir_lib + 'sub_ftn/rv_match.so'
			lset = LONARR(20) & dset = DBLARR(20)
				...
				IF settings.R_orgver EQ 0L THEN lset(18) = 100 ;; For YZiCS ver
				IF settings.R_longint EQ 0L THEN lset(19) = 100 ;; For logn int ID
	...
	SAVE, filename=dir_data + 'rv_ptmatch.sav', output
	RETURN, PTR_NEW(output,/no_copy)
ENDIF
END
